<div>
    <div class="flex items-start justify-between mb-6">
        <div class="flex items-center gap-3">
            ...
            <div>
                <div class="flex items-center gap-2 mb-0.5">
                    <flux:heading size="xl">Edit Question</flux:heading>
                    <flux:badge color="yellow" size="sm" icon="pencil">Draft</flux:badge>
                </div>
                <flux:subheading>
                    Type: <strong>{{ str_replace('_', ' ', ucfirst($questionType->question_type)) }}</strong>
                    &middot; {{ $questionType->role_description }}
                </flux:subheading>
            </div>
        </div>
        ...
        <flux:modal.trigger name="confirm-delete">
            <flux:button variant="danger" icon="trash">Delete Question</flux:button>
        </flux:modal.trigger>
    </div>
    ...
    <flux:modal name="confirm-delete" class="max-w-sm">
        <div class="space-y-4">
            <div>
                <flux:heading size="lg">Delete question?</flux:heading>
                <flux:subheading class="mt-1">
                    This will permanently delete the question and all its options. This cannot be undone.
                </flux:subheading>
            </div>
            <div class="flex gap-3">
                <flux:button variant="danger" wire:click="delete" class="flex-1">
                    Yes, delete it
                </flux:button>
                <flux:modal.close>
    ...
    <div class="max-w-3xl space-y-10">
        ...
        <section>
            ...
            <div class="space-y-4">
                ...
                <flux:field>
                    ...
                    <flux:input wire:model.live="questionHint" placeholder="A short hint shown to users…" />
        ...
        <section>
            ...
            <flux:subheading class="mb-4">
                Use the arrows to set display order. Minimum 2 options required.
            </flux:subheading>
        ...
        <section>
            ...
            <flux:subheading class="mb-4">
                @if ($questionType->evaluation_mode === 'manual')
                    This question is graded manually. Optionally provide a model answer for reference.
                @else
                    Set the correct answer. Required to publish.
                @endif
            </flux:subheading>

            {{-- Binary --}}
            @if ($questionType->question_type === 'binary')
                <div class="flex gap-3">
                    <button
                        type="button"
            ...
            {{-- Single choice --}}
            @elseif ($questionType->question_type === 'single_choice')
                @if (count($options) >= 2)
                    <div class="space-y-2">
            ...
            {{-- Multiple choice --}}
            @elseif ($questionType->question_type === 'multiple_choice')
                @if (count($options) >= 2)
                    <div class="space-y-2">
                        @foreach ($options as $option)
                            <label
            ...
            {{-- Number input --}}
            @elseif ($questionType->question_type === 'number_input')
            ...
            @elseif ($questionType->question_type === 'text_input')
                <flux:field>
                    <flux:label>
                        Model answer
                        <flux:badge size="sm" color="zinc" class="ml-1">Optional</flux:badge>
